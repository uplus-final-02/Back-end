...
Elasticsearch, Redis, 한국어 형태소 분석, 초성 검색, 벡터 검색을 조합해 **검색 품질과 운영 안정성**을 함께 개선했습니다.
...
- **하이브리드 검색 품질 개선**
  ...
  - N-gram 기반 부분 일치 검색
  - 오타 허용 fuzzy 검색
  - 사용자 선호 태그 기반 HNSW kNN 벡터 검색
  - `function_score` 기반 조회수/북마크 수 가중치 반영

- **한국어 검색 최적화**
  - 초성 검색 지원
  - OTT 도메인 특화 사용자 사전 40개 이상 등록
  - 동의어 사전 적용  
    (예: 드라마/시리즈, 영화/무비, 공포/호러, 로맨스/멜로)

- **캐시 최적화**
  - Redis Cache-Aside 기반 검색 결과 캐싱
  - 비개인화 결과를 캐시하고, 로그인 사용자의 `RELATED` 정렬만 후처리 재정렬
  - Redis 장애 시 ES 직접 조회하는 **fail-open 전략** 적용

- **자동완성 안정성 개선**
  - 자동완성 전용 캐시 TTL 분리 운영
  - Redis 또는 ES 장애 시 빈 결과 반환으로 UX 저하 최소화

**검색 쿼리 전략**

메인 검색은 다음 요소를 함께 반영합니다.
...
이를 통해 단순 키워드 매칭을 넘어 **정확도 + 탐색성 + 개인화**를 동시에 확보했습니다.

### 6-2-2. 크리에이터 콘텐츠 검색 확장

기존 OTT 본편 검색 외에 **크리에이터 영상 검색 기능**을 추가했습니다.

**제공 기능**

- 특정 OTT 콘텐츠에 연결된 크리에이터 영상 목록 조회
- 특정 업로더(크리에이터)의 영상 목록 조회
- 크리에이터 영상 썸네일 업로드

**검색 API**

```text
GET /api/search/creator?parentContentId={id}
GET /api/search/creator/user?uploaderId={id}
POST /api/user/contents/{id}/thumbnail

**구현 특징**

- Elasticsearch 우선 조회
- 장애 시 DB Fallback 또는 빈 결과 반환
- 업로더 본인만 썸네일 업로드 가능하도록 소유자 검증 적용
- 크리에이터 썸네일이 없으면 부모 콘텐츠 썸네일을 fallback으로 사용

이를 통해 플랫폼이 **정식 OTT 콘텐츠 + UGC(크리에이터 콘텐츠)** 를 함께 다룰 수 있는 구조로 확장되었습니다.

### 6-2-3. 검색 운영 안정성 및 실시간 동기화

검색 인덱스가 실제 DB 상태를 빠르게 반영할 수 있도록 **실시간 Elasticsearch 동기화 파이프라인**을 구축했습니다.

**실시간 동기화**

- `ContentRealtimeSyncScheduler`
  - OTT 콘텐츠 변경 사항을 주기적으로 감지하여 ES 인덱스 반영
- `UserContentRealtimeSyncScheduler`
  - 크리에이터 콘텐츠 변경 사항을 별도로 감지하여 ES 인덱스 반영

**안정성 설계**

- `updatedAt + id` 복합 커서 기반 증분 동기화
- 동일 시각 데이터 누락 및 중복 최소화
- 워터마크 3계층 저장
  - Redis
  - MySQL
  - In-Memory
- 청크 단위 배치 처리
- 실패 건은 `es_sync_failure` DLQ에 적재
- 재시도 스케줄러를 통해 일정 횟수까지 자동 복구

**장애 대응**

- ES 검색 실패 시 DB LIKE 검색으로 Fallback
- 자동완성 실패 시 빈 배열 반환
- Redis 장애 시 캐시 미스로 간주하고 직접 조회
- ES 동기화 실패 시 전체 스케줄러를 중단하지 않고 실패 건만 분리 처리

이를 통해 검색 품질뿐 아니라 **장애 복원력과 데이터 정합성**도 함께 강화했습니다.

### 6-2-4. 검색 리팩토링 및 코드 리뷰 반영

검색 기능 고도화 과정에서 PR 리뷰 피드백 **12건**을 반영하며 구조를 개선했습니다.

**주요 개선 내용**

- N+1 쿼리 제거
- fetch join 기반 `LazyInitializationException` 가능성 해소
- `volatile` 누락 보완으로 비동기 상태 가시성 확보
- JPQL 상태값 하드코딩 제거
- `_score` 기반 정렬 fallback 시 WARN 로그 추가
- OOM 위험이 있는 Slice 배치를 커서 기반 처리로 전환
- self-invocation 문제를 재시도 실행기 분리로 해결
- 검색 로그 무한 증가 방지를 위한 배치 삭제 스케줄러 추가
- 인덱스명 상수화 및 캐시 키 인코딩 개선

이를 통해 검색 기능을 단순 구현 수준에서 끝내지 않고, **성능, 유지보수성, 장애 대응, 운영 편의성**까지 고려한 구조로 리팩토링했습니다.
...
사용자 개인화 경험 강화를 위해 **프로필 이미지 관리**, **북마크(찜) 시스템**, **플레이리스트 조회** 기능을 구현했습니다.  
단순 CRUD를 넘어, 스토리지 부하를 줄이는 업로드 구조와 시청 이력 연계를 함께 고려해 **마이페이지 중심 UX**를 개선했습니다.

### 10-1. Presigned URL 기반 프로필 이미지 업로드

프로필 이미지 업로드 시 백엔드 서버를 경유하는 방식은 트래픽 증가 시 메모리 및 네트워크 대역폭에 부담을 줄 수 있습니다.  
이를 해결하기 위해 **Presigned URL 기반 직접 업로드 구조**를 적용했습니다.

**동작 방식**

```text
1. 클라이언트 → GET /api/profile/image/presigned-url
2. 백엔드 → MinIO Presigned URL 발급
3. 클라이언트 → MinIO에 직접 PUT 업로드
4. 클라이언트 → PATCH /api/profile/image 로 최종 반영 요청
5. 백엔드 → 파일 경로/확장자/소유권 검증 후 사용자 프로필 이미지 갱신

**구현 포인트**

- 백엔드는 파일 자체를 중계하지 않고 Presigned URL만 발급
- 업로드 트래픽을 스토리지로 직접 분산해 서버 부하 최소화
- 최종 반영 단계에서 파일 확장자 및 사용자 소유 경로를 검증해 안전성 확보

### 10-2. 북마크(찜) 시스템

사용자가 관심 있는 콘텐츠를 저장하고 다시 탐색할 수 있도록 **북마크 기능**을 구현했습니다.

**제공 기능**

- 북마크 추가  
  `POST /api/users/me/bookmarks/{contentId}`
- 북마크 삭제  
  `DELETE /api/users/me/bookmarks/{contentId}`
- 북마크 목록 페이징 조회  
...
- 사용자별 북마크 데이터 분리
- 중복 북마크 방지
- 마이페이지에서 페이지네이션 기반 조회 지원

### 10-3. 플레이리스트형 북마크 조회

단순 목록 나열을 넘어, 콘텐츠 타입별로 북마크를 분류해서 볼 수 있는 **플레이리스트 조회 API**를 구현했습니다.

**API**

```text
GET /api/users/me/bookmarks/playlist

**특징**

- 콘텐츠 타입(예: 영화, 시리즈) 기준 그룹화 조회
- 저장한 콘텐츠를 목적에 맞게 다시 탐색할 수 있도록 UX 개선

### 10-4. 시청 이력 연계 UX 개선

북마크 조회 시 시청 이력 정보를 함께 활용해 사용자가 마이페이지에서 바로 **이어보기**를 할 수 있도록 UX를 개선했습니다.

**개선 내용**

- 북마크 콘텐츠 조회 시 시청 이력 정보를 함께 제공
- 마지막 시청 위치 기반 이어보기 경험 지원
- 저장 기능과 재생 경험이 자연스럽게 연결되도록 구성
